...
exports.bodySchema = {
    ...
    email: yup.string().required().email().trim().when('$user', (user, schema) => user ? schema.databaseUnique('User', user.id) : schema).label('Email'),
    ...
    leagueOfLegendsUsername: yup.string().required().min(3).max(16).trim().when('$user', (user, schema) => user ? schema.databaseUnique('User', user.id) : schema).label('Username do League of Legends'),
